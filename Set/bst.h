...
namespace custom
{
    ...
    typename BST <T> ::iterator BST <T> ::erase(iterator& it)
    {
        ...
        else
        {
            ...
            // If the IOS is not direct sibling, then put it in the place of pDelete.
            ...
            if (root == pDelete) // What if it was the root?
                root = pIOS;
    ...
    void BST <T> ::deleteNode(BNode*& pDelete, bool toRight)
    {
        ...
        // If we are not the parent, hook ourselves into the exisiting tree.
        if (pDelete != root)
        {
    ...
    void BST <T> ::BNode::balance()
    {
        ...
        // Case 3: If the parent and the aunt are red, then repaint them black and
        if (pAunt != nullptr && pAunt->isRed == true)
        {
            pGranny->isRed = true; // make granny red
            pParent->isRed = false; // make parent black
            pAunt->isRed = false; // make aunt black
            pGranny->balance(); // balance granny
            return;
        }

        // Case 4: If the parent is red and the aunt is black, then we need to rotate
        BNode* pHead = nullptr;
